...
const floodFill = ({
    ...
    startY,
    startColor,
}: FloodFillOptions): Uint8ClampedArray => {
    ...
    const matchSpanStartColor = colorMatcherSpanStart(image, ...startColor as [number, number, number, number])
    // for all other cases, we look for the start colour exactly
    const matchStartColor = colorMatcherIgnoreAlpha(image, ...startColor as [number, number, number, number])

    // check if we're trying to fill with the same colour, if so, stop
    const matchFillColor = colorMatcher(image, ...[...fillColor, OPAQUE] as [number, number, number, number])
    if (matchFillColor((startYCoord * width + startXCoord) * PIXEL)) {
        return image
    }
    // begin with our start pixel
    const pixelStack: [number, number][] = [[startXCoord, startYCoord]]
