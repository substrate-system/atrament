import type Atrament from '../dist/esm/index.js'
import type { CurrentStroke } from '../dist/esm/index.js'
import type { Point } from '../dist/esm/mouse.js'

const recordedStroke: Partial<CurrentStroke> = {}

const waitUntil = (reference: number, time: number): Promise<void> => {
...
export const setRecorded = (stroke: CurrentStroke): void => {
    Object.assign(recordedStroke, stroke)
}

export const playRecorded = async (atrament: Atrament): Promise<void> => {
    if (!recordedStroke.segments || !recordedStroke.weight || !recordedStroke.mode ||
      !recordedStroke.smoothing || !recordedStroke.color || recordedStroke.adaptiveStroke === undefined) {
        return
    }
    ...
    let prevPoint: Point = recordedStroke.segments[0].point
